...
**Node configuration:**
1. Search for **Cal.com** and pick **Cal.com Trigger** from the trigger list
2. Select your credential
3. **Events** -- select **Booking Created**
4. Click **Listen for test event**
...
**What happens behind the scenes:**
- n8n credential test calls `GET api.cal.com/v1/memberships?apiKey=test` --
  proxy serves a fixture
- n8n calls `GET api.cal.com/webhooks?apiKey=test` to check for existing
  webhooks -- proxy returns an empty list
- n8n calls `POST api.cal.com/webhooks?apiKey=test` with a randomly generated
  64-char hex `secret` in the body -- proxy returns a mock webhook **and**
  captures the `secret` to `~/.n8n-node-mocker/cal-secret.txt`
- `webhook fire` auto-detects the captured secret and uses it to compute
  the HMAC-SHA256 hex signature in the `X-Cal-Signature-256` header

**Gotchas:**
- Cal.com uses a **dynamic secret** generated by n8n (similar to Calendly,
  GitLab, Figma). The proxy captures it automatically via service hook.
- The API key is sent as a **query parameter** (`?apiKey=test`), not a header.
  The proxy strips query params for fixture matching, so this works fine.
- The host field in credentials determines which hostname the proxy intercepts.
  Use `https://api.cal.com` for the default fixtures to match.
- Cal.com has two API versions: v1 uses `/hooks`, v2 uses `/webhooks`. The
  default for new nodes is v2.

**Available events:** `BOOKING_CREATED`, `BOOKING_CANCELLED`, `BOOKING_RESCHEDULED`
...
**Node configuration:**
1. Search for **Formstack** and pick **Formstack Trigger** from the trigger list
2. Authentication: **Access Token**
3. Select your credential
4. **Form Name or ID** -- select **Contact Form** from the dropdown
   (the proxy serves a fixture for this)
5. Leave **Simplify** enabled (default)
...
**What happens behind the scenes:**
- n8n calls `GET www.formstack.com/api/v2/form.json` to populate the form
  dropdown -- proxy serves a fixture with mock forms
- n8n calls `GET www.formstack.com/api/v2/form/<formId>/webhook.json` to check
  for existing webhooks -- proxy returns an empty list (parent-path match)
- n8n calls `POST www.formstack.com/api/v2/form/<formId>/webhook.json` with a
  randomly generated 64-char hex `hmac_secret` in the body -- proxy returns a
  mock webhook ID **and** captures the `hmac_secret` to
  `~/.n8n-node-mocker/formstack-secret.txt`
- `webhook fire` auto-detects the captured secret and uses it to compute the
  HMAC-SHA256 hex signature with `sha256=` prefix in the `X-FS-Signature` header

**Gotchas:**
- Formstack uses a **dynamic secret** generated by n8n via
  `randomBytes(32).toString('hex')`. The proxy captures it automatically via
  service hook -- same pattern as Calendly, GitLab, Figma.
- The signature format is `sha256=<hex_digest>` (prefix + hex).
- Formstack also supports OAuth2 auth, but Access Token is simpler for testing.
- The API base is `www.formstack.com/api/v2/` -- note `www.` prefix.

**Available events:** `submission`
...
> **Important:** The URL field determines which hostname the proxy intercepts.
> Use `https://test.mautic.net` so the built-in fixtures match. A self-hosted
> instance would need its own hostname-specific fixtures.

**Node configuration:**
1. Search for **Mautic** and pick **Mautic Trigger** from the trigger list
2. Authentication: **Credentials**
3. Select your credential
4. **Event Names or IDs** -- select **When a contact is created** from the
   dropdown (the proxy serves a fixture for this)
5. Click **Listen for test event**
...
npx n8n-node-mocker webhook fire \
  --service mautic \
  --url http://localhost:5678/webhook-test/<id>/webhook \
  --event lead_post_save_new
```

**What happens behind the scenes:**
- n8n credential test calls `GET test.mautic.net/api/users/self` -- proxy serves
  a fixture with a mock user (HTTP Basic auth, username/password)
- n8n calls `GET test.mautic.net/api/hooks/triggers` to populate the events
  dropdown -- proxy returns a fixture with available event triggers
- n8n checks `webhookData.webhookId` -- on first activation it's undefined so
  checkExists returns false immediately (no API call)
- n8n calls `POST test.mautic.net/api/hooks/new` with a randomly generated
  64-char hex `secret` in the body -- proxy returns a mock webhook ID **and**
  captures the `secret` to `~/.n8n-node-mocker/mautic-secret.txt`
- `webhook fire` auto-detects the captured secret and uses it to compute the
  HMAC-SHA256 base64 signature in the `Webhook-Signature` header

**Gotchas:**
- Mautic uses a **dynamic secret** generated by n8n (same pattern as Formstack,
  GitLab). The proxy captures it via service hook -- matched by path pattern
  (`/api/hooks/new`) rather than hostname since the URL is user-provided.
- The signature is HMAC-SHA256 with **base64 encoding** (not hex like most services).
- Mautic uses **HTTP Basic auth** (username/password), not API key or Bearer token.
- The URL in credentials must match the fixture hostname. Use `https://test.mautic.net`.
- Event IDs use Mautic's internal naming (e.g. `mautic.lead_post_save_new`).

**Available events:** `lead_post_save_new`, `form_on_submit`
...
- For static secrets: ensure the credential in n8n and the mocker config
  both use `test`
- For dynamic secrets (Cal.com, Calendly, Formstack, GitLab, Figma, Mautic, Netlify, Taiga):
